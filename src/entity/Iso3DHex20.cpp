...
QuickMatrix<20, 3> Iso3DHex20::getLocalShapeDerivative(Precision r, Precision s, Precision t) {
    ...
    QuickMatrix<20, 3> local_shape_derivative {};
    ...
    for (int i = 0; i < 8; i++) {
        ...
        local_shape_derivative(i, 0) =
            -1.0 / 8.0 * sum_s * sum_t * (sign_r1 * sum_1 + sign_r2 * sum_r);
        local_shape_derivative(i, 1) =
            -1.0 / 8.0 * sum_r * sum_t * (sign_s1 * sum_1 + sign_s2 * sum_s);
        local_shape_derivative(i, 2) =
            -1.0 / 8.0 * sum_r * sum_s * (sign_t1 * sum_1 + sign_t2 * sum_t);
    }
    ...
    for (int i = 8; i < 20; i++) {
        ...
        local_shape_derivative(i, 0) = 1.0 / 4.0 * s1 * s2 * t1 * t2 * (r1 * sign_r2 + r2 * sign_r1);
        local_shape_derivative(i, 1) = 1.0 / 4.0 * r1 * r2 * t1 * t2 * (s1 * sign_s2 + s2 * sign_s1);
        local_shape_derivative(i, 2) = 1.0 / 4.0 * r1 * r2 * s1 * s2 * (t1 * sign_t2 + t2 * sign_t1);
    }

    return local_shape_derivative;
}
QuickMatrix<6, 60> Iso3DHex20::computeStrainDisplacementRelationFromSource(QuickMatrix<20,3> b_help) {
...
QuickMatrix<20, 1> Iso3DHex20::getShapeFunction(Precision r, Precision s, Precision t) {
    QuickMatrix<20, 1> res {};
    ...
    for (int i = 0; i < 8; i++) {
        ...
        res(i, 0)         = -1.0 / 8.0 * sum_r * sum_s * sum_t * sum_1;
    }
    // remaining 12 nodes
    for (int i = 8; i < 20; i++) {
        Precision r1 = (((i + 1) / 2) % 2 == 0 ? rm : rp);
        ...
        res(i, 0)    = 1.0 / 4.0 * r1 * r2 * s1 * s2 * t1 * t2;
    }
    return res;
}
QuickMatrix<20, 3> Iso3DHex20::getNodeLocalCoordinates() {
    QuickMatrix<20, 3> res{};

    // first 8 nodes
    for(int i = 0; i < 8; i++){
        res(i,0) = ((i + 1) / 2) % 2 == 0 ? -1:1;
        res(i,1) = ((i + 0) / 2) % 2 == 0 ? -1:1;
        res(i,2) = ((i + 0) / 4) % 2 == 0 ? -1:1;
    }

    // second 8 nodes
    for(int i = 8; i < 16; i++){
        res(i,0) = i % 2 == 0 ? 0 : ((i-1) % 4 == 0 ? 1:-1);
        res(i,1) = i % 2 == 1 ? 0 : ((i  ) % 4 == 0 ?-1: 1);
        res(i,2) = ((i + 0) / 4) % 2 == 0 ? -1:1;
    }
