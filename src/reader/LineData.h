...
struct LineData{
    ...
    LineData& operator=(const std::string& p_line){
        ...
        this->line.erase(std::remove_if(this->line.begin(), this->line.end(), isspace), this->line.end());
    ...
    std::string requireValue(const std::string &key){
        for(int i = 0; i < csv_count; i++){
            if (csv[i].rfind(key, 0) == 0) { // pos=0 limits the search to the prefix
                ...
                std::string value = csv[i].substr(csv[i].find('='), csv[i].length());
                return value;
            }
    ...
    std::string commandName(){
        return csv->substr(1,csv->length());
    }
};
